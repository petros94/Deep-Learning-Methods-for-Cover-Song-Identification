...
def train_triplet_loss(
    ...
    batch_all_miner = miners.TripletMarginMiner(
        margin=margin, type_of_triplets="all", distance=distance
    )
    batch_hard_miner = miners.TripletMarginMiner(
        margin=margin, type_of_triplets="hard", distance=distance
    )
    ...
    losses_history = {"epoch": [], "train": [], "valid": []}
    ...
    for epoch in range(n_epochs):
        ...
        for i in range(len(train_set)):
            ...
            if i % 16 == 0:
                print(
                    f"batch {i}/{train_batches}, loss: {loss.item()}, triplets: {miner.num_triplets}"
                )
